...
;;
;;Init del generatore di numeri casuali, inizializzato all'attuale numero di micro secon
(mtfa-rand-seed (mtfa-micros))
;;
;;Initialization terminated. Program started
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;La funzione che scatta in corrispondenza all'hook ha due parametri
;;actionl (che è una lista) e pbuf (che è il puntatore alla sessione HTTP)
;;Esempio di attivazione: GET "http://127.0.0.1:9999/msvc?nome=franco&servizio=accesso"
(defun Manage::MSCV (actionl pbuf)
  (eis::GiveErrorHTTP401) =>
  ;; actionl:: i parametri passati in sdc.ars che voglio stampare
  ;; pbuf:: è il puntatore al buffer che ci permette di accedere direttamente a tutti gli elementi della comunica
  (Show "actionl: " actionl ". pbuf: " pbuf)

  ;; (va::vaeng::GetFromSet ) ;;per prendere elementi dai set passati, come actionl, dando una key
  ;; (va::vaeng::AddToSet ) ;;per aggiungere
  ;; (va::vaeng::DelromSet ) ;;per eliminare

  (Show "Campo host: " (mtfa-eis-get-current-host pbuf)
	", " (mtfa-eis-get-current-query pbuf) ;;prende tutta la parte query! sia dalla get che dalla post
	". Nome: " (mtfa-eis-get-value-current-query pbuf "nome") ;; prende solo il valore di "nome" dalla query
  ...
  (eis::GiveHTTPTextAnswer "risultato=OK")
  )

;;Add HOOK, fa scattare la funzione di gestione Manage::MSCV per la stringa "MICROSVC" (definita in sdc.ars)
(eis::function-pointer-add "MICROSVC" Manage::MSCV)
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

#| 
Fri Apr 30 16:27:06Z 2021 actionl: (nome_utenti). pbuf: #<pointer 0x7f5e5c002060>
Fri Apr 30 16:27:06Z 2021 Campo host: 127.0.0.1, (). Nome: (). Headers: GET /msvc HTTP/1.1
TE: deflate,gzip;q=0.3
Connection: TE, close
Host: 127.0.0.1:9999
User-Agent: lwp-request/6.47 libwww-perl/6.47
...
(defun Manage::API1 (actionl pbuf)
  ...
  #|Tutti gli elementi che possono essere utilizzati dalla sessione
            ...
            mtfa-eis-get-current-query ;; prende tutta la query string, sia per post sia per get
